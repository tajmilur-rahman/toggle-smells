...
# -------- Defaults --------
INSTALL_DIR="${HOME}/Repos/toggle-smells"
BRANCH="main"
PYTHON_BIN="python3"
REPO_URL="https://github.com/tajmilur-rahman/toggle-smells.git"
...
# -------- Enter ts-detector module --------
if [[ -d "$INSTALL_DIR/ts-detector" ]]; then
  cd "$INSTALL_DIR/ts-detector"
else
  cd "$INSTALL_DIR"
fi
